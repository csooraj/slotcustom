...
define(['jquery', 'rngModule', 'audioModule'], function($, rngModule, audioModule) {
    ...
    if(credit<=0){
			...
			document.getElementById('button-spin').style.pointerEvents = 'auto';
		}
});
					...
					if(reelSpin[index] === false && reelSpinning[index] === true) {
						if(reelPosition[index] === reelNumbers[index] || reelHold[index]) {
							...
							if(!reelSpinning[0] && !reelSpinning[1] && !reelSpinning[2]) {
								...
								if(reelHoldNumbers[0]===reelHoldNumbers[1]===reelHoldNumbers[2]){
									credit = credit + 20;
									...
									if(credit === 0){
										document.getElementById("credits").innerHTML = "BUY CREDITS: ";
										document.getElementById('button-spin').style.pointerEvents = 'none';
									}else{
										document.getElementById("credits").innerHTML = "CREDITS: "+credit;
									}
